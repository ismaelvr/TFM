...
def load_step(self, gs):
    ...
    self.step_label = tk.Label(self.step_text_frame, font='Consolas 16 bold', textvariable=self.step_val_str)
    self.step_result_text_frame = tk.Frame(self.gs_frame)
    self.step_result_val_str = tk.StringVar()
    self.step_result_label = tk.Label(self.step_result_text_frame, font='Consolas 16 bold', textvariable=self.step_result_val_str)
...
def previous_g(self):
    if self.actual_g > 0:
        ...
        b2_int.update_sigma_color(self, mi = self.actual_block_rounds[self.actual_round - 1].gs[self.actual_g].m_pos[0], mj = self.actual_block_rounds[self.actual_round - 1].gs[self.actual_g].m_pos[1])
...
def load_vs(self):
    ...
    for i in range(total_rows):
            for j in range(total_columns):
                self.e = tk.Entry(self.vs_frame, justify=tk.CENTER, width=5, font=('Consolas',11,'bold'))
