...
def create_frames(self):
    ...
    self.main_frame.pack(pady = 15, padx = 15)
...
def create_teoric_window(self):
    ...
    # canvas.imageList.append(img)
        ...
        self.teoric_text_label = tk.Label(self.teoric_text_frame, font='Arial 13', wraplength=770)
        self.teoric_text_label.config(text="En la ventana de bloques puedes desplazarte a través de los bloques. En la tabla h aparecen los valores de h al finalizar la ejecución del bloque actual. Las h del último bloque en hexadecimal son el hash final. Poniendo el cursor encima de una de las h se puede ver los cálculos realizados para su obtención. Con el botón Hex/Dec puedes alternar entre los valores en hexadecimal y decimal.\n\n"
        "En la ventana de rondas puedes desplazarte dentro de cada una de las 12 rondas de cada bloque. Esa tabla muestra los valores de a, b, c y d obtenidos tras la ejecución de cada una de las 8 funciones G de la ronda actual. El texto en rojo indica cuales son los valores de la G actual.\n\n"
        "En la ventana de función G puedes desplazarte dentro de cada una de las 8 funciones G de y, a su vez, desplazarte dentro de cada uno de los 8 pasos de cada función G. En los pasos en los que se realizan operaciones xor y desplazamientos se puede ver el cálculo poniendo el ratón encima.\n\n"
        ...
        self.teoric_text_frame.pack(pady = 15)

        self.teoric_text_frame2 = tk.Frame(self.teoricWindow, pady = 5)

        def on_closing():
            self.teoricWindow.destroy()
            self.teoricWindow = None

        self.teoricWindow.protocol("WM_DELETE_WINDOW", on_closing)
...
def create_sigma_window(self, sigma):
    ...
    for i in range(total_rows-2):
        for j in range(total_columns):
            ...
            self.e = tk.Entry(self.sigmaWindow, justify=tk.CENTER, width=2, font=('Consolas',11,'bold'))
            self.e.grid(row=i, column=j)
            self.e.insert(tk.END, sigma[i][j])
            self.entry_grid_sigma[i][j] = self.e
            self.e.config( state="readonly")
    self.actual_round = 1
    update_sigma_window(self, mi = self.actual_block_rounds[0].gs[0].m_pos[0], mj = self.actual_block_rounds[0].gs[0].m_pos[1])
...
def create_IV_window(self, IVs):
    ...
    for i in range(total_rows):
        self.e = tk.Entry(self.IVWindow, justify=tk.CENTER, width=20, font=('Consolas',11,'bold'))
...
def create_rounds_window(self):
    ...
    self.rounds_label = tk.Label(self.rounds_text_frame, font='Consolas 16 bold', textvariable=self.round_val_str)
...
def create_gs_window(self):
    ...
    self.gs_label = tk.Label(self.gs_text_frame, font='Consolas 16 bold', textvariable=self.gs_val_str)
    ...
    self.gs_act_vals_label = tk.Label(self.gs_act_vals_frame, font='Consolas 16 bold', textvariable=self.gs_act_vals_str)
...
def create_blocks_window(self):
    ...
    self.blocks_label = tk.Label(self.blocks_text_frame, font='Consolas 16 bold', textvariable=self.block_val_str)
    ...
    self.hs_label = tk.Label(self.hs_text_frame, font='Consolas 11 bold')
...
def update_sigma_window(self, mi , mj):
    l = 0    
    for i in range(len(self.entry_grid_sigma)):
        ...
        for j in range(0, len(self.entry_grid_sigma[0]), 2):
            ...
            self.e_ttp = ttp.ToolTip(self.entry_grid_sigma[i][j], self.actual_block_rounds[(l % 10)].gs[k].m[0])
            self.e_ttp = ttp.ToolTip(self.entry_grid_sigma[i][j + 1], self.actual_block_rounds[(l % 10)].gs[k].m[1])   
            k = k + 1 
        l = l + 1

def update_sigma_color(self, mi , mj):
    for i in range(len(self.entry_grid_sigma)):
        for j in range(len(self.entry_grid_sigma[0])):   
            self.entry_grid_sigma[i][j].config(fg="black")     
    self.entry_grid_sigma[(self.actual_round - 1) % 10][mi].config(fg="red")
    self.entry_grid_sigma[(self.actual_round - 1) % 10][mj].config(fg="red")
